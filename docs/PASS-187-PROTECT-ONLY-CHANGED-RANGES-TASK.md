...
音訊完整管線回歸已完成：`irregular_measure_count` 維持 14（跟 Pass 186
完全相同的小節位置），證實無效保護區段的過濾對最終輸出沒有影響——真正的
交界處接縫來自「真的需要保護」的區段本身。這個方向需要的後續修法由
Pass 188 起接續處理，詳見 `docs/BT-BUILD-PROGRESS.md`。
...
**結論**：`irregular_measure_count` 維持 14，**沒有改善**。原因確認如 第 3 節末
所預判：Pass 187 只讓保護清單更精準（37 → 15 段），但 SteadyPercussionCountAnchorNode
每次 `_apply_anchor` 在 `base_idx` 開始往後重標 1-2-3-4 時，`base_idx` 之前
的最後一個小節相位若對不上，`MeasureMapNode` 仍會切出一個只有 1-3 拍的破碎小節。
這是交界處相位跳躍問題，需要獨立的 Pass 188 處理。

詳見 **docs/PASS-188-BOUNDARY-PHASE-SMOOTHING-TASK.md**（待建立）。
