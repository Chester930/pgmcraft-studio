...
### 4.3.1 根因分析（第二層：實際試聽 + 重播節點邏輯後追到的確切機制）

使用者實際試聽處理組的 `mix_with_click.wav` 後回報：**7.1s-13.5s、16.1s-19.2s
兩段完全沒有 click 聲**（累計約 9.5 秒）。這比 BPM 跳動更嚴重——不是「拍點跟
音樂對不上」，是「拍點整段消失」。

追查方式：對照 `GapReinforcementNode` 自己匯出的診斷紀錄
（`reports/gap_reinforcement/beats.json`），確認它執行完畢當下，4.4s-21.8s
這段其實有連續、規律的拍點（433 個拍點，間隔對應約 166 BPM）——證明拍點消失
**不是 GapReinforcementNode 自己刪掉的**，是它之後的某個精修節點造成的。

用真實的 433 個拍點資料，原封不動重播 `ViterbiTempoSmoothingNode` 的實際演算法
（純陣列運算，不需要音訊），精確重現了消失現象：

```
idx=5   原始時間=4.389s  → 修正後=2.589s
idx=6   原始時間=5.178s  → 修正後=2.949s
...（連續 21 個拍點都被拖走）...
idx=25  原始時間=18.947s → 修正後=9.789s
idx=26  原始時間=19.280s → 修正後=19.280s（從這裡開始恢復正常）
```

**確切機制**：`ViterbiTempoSmoothingNode` 算出全曲拍點間隔的中位數（此曲為
0.36 秒 ≈ 166.7 BPM），逐一檢查每個拍點間隔，只要跟中位數差超過 20%
（`tolerance_pct`）就判定是「孤立突變離群拍點」，強制改寫成
`前一拍時間 + 全曲中位數間隔`。這個設計假設離群值是**零星、孤立**的單一雜訊
點——但 `GapReinforcementNode` 補強出來的整段缺口，因為局部證據（貝斯/和弦）
推算出的節奏本來就跟全曲中位數不同，產生的是**一整串連續 21 個「跟中位數不同」
的拍點**，不是孤立的。節點把這整串都當離群值，逐一用「前一拍（可能已經被前
一次修正過）+ 固定中位數間隔」去改寫——修正逐拍疊加，把整段拍點越拖越早，
最終被壓縮進一個只有原本一半長的時間窗，原本的時間窗就變成完全沒有拍點的
空白。

**這是兩個節點的假設互相牴觸，不是單一節點的獨立 bug**：`GapReinforcementNode`
產生的是「一整段跟全曲節奏不同、但內部自己連貫」的區塊；`ViterbiTempoSmoothingNode`
假設所有跟全曲中位數不同的都是零星雜訊，用同一套「拉回全曲中位數」的邏輯去
處理，兩者疊加才炸出這個問題。這比 4.3 節「品質守門沒檢查邊界連貫性」的說法
更精確——**真正的下游破壞點就是 `ViterbiTempoSmoothingNode`**，不是泛指「某個
精修節點」。
...
- 修正 `ViterbiTempoSmoothingNode` 誤判連續補強區塊為孤立離群值、造成拍點整段
  消失的問題（見 4.3.1 節）。討論中的方向（尚未選定、尚未實作）：
  a) 讓該節點改用**區段內局部中位數**而非全曲中位數判斷離群值；
  b) 讓 `GapReinforcementNode` 補強過的區間寫進類似 `snap_exclusion_zones`
     的排除清單（比照 `DrumFillDetectionNode` 的做法），讓 Viterbi 略過；
  c) 調整節點順序或介面，避免補強結果被假設「全曲統一速度」的平滑節點處理。
