r"""
Pass 190 — 下游 Downbeat 衛兵網格平移修復後的預設管線真實資料回驗
"""
...
REGRESSION_ROOT = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "outputs", "pass190_default_pipeline_reverify"
)
...
def ensure_shared_stems_cache() -> None:
    if os.path.exists(SHARED_STEMS_CACHE):
        return
    print(f"[Pass190回驗] 建立共用 stems 快取：{SHARED_STEMS_CACHE}")
    os.makedirs(os.path.dirname(SHARED_STEMS_CACHE), exist_ok=True)
    p189_cache = r"D:\Users\666\Desktop\UVR5 音檔\自動節拍器\.claude\worktrees\pass171-multi-variant-harness\outputs\pass189_default_pipeline_reverify\_shared_stems_cache\stems"
    src = p189_cache if os.path.exists(p189_cache) else os.path.join(SOURCE_PROJECT_DIR, "stems")
...
def main():
    ...
    print(f"[Pass190回驗] stems 重用方式：{stems_mode}")

    print(f"\n=== [Pass190回驗] 執行 target_stage=module3（含 Pass 190 網格平移修復）===")
    t0 = time.time()
    ...
    print(f"Pass190 這次結果: {stats['total_measures']} 小節（差黃金基準 {diff['total_measures']:+d}） / "
