"""PASS-212: comprehensive comparison of Verse 1 (perfect baseline, 42/42)
against Chorus 1 (imperfect, 41/42) using the CLEAN baseline code (no
speculative fixes applied) -- captures every tick from 25s to 148s so the
two segments can be directly compared for whatever distinguishes them.

Audit-only: monkeypatches only node execute() methods (never __init__ /
stall_limit), restores everything in finally.
"""
...
OUTPUT_ROOT = os.path.join(ROOT, "outputs", "pass212_verse_chorus_comparison")
STEMS_SOURCE = os.path.join(ROOT, "outputs", "pass203_evidence_fusion_diagnosis", AUDIO_NAME, "stems")
TRACE_PATH = os.path.join(ROOT, "scratch", "pass212_verse_chorus_trace.jsonl")
...
def main():
    if not os.path.exists(AUDIO_PATH):
        print(f"[PASS-212-vc] blocked: missing {AUDIO_PATH}")
        return 1
    reuse_stems_cache()
    ...
    print(f"[PASS-212-vc] elapsed={elapsed:.1f}s watched_ticks={len(trace)} trace={TRACE_PATH}")
    return 0
