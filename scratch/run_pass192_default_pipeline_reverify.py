r"""
Pass 192 — MeasureMapNode 過長小節網格拆分與防膨脹修復真實資料回驗
"""
...
REGRESSION_ROOT = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "outputs", "pass192_default_pipeline_reverify"
)
...
def ensure_shared_stems_cache() -> None:
    if os.path.exists(SHARED_STEMS_CACHE):
        return
    print(f"[Pass192回驗] 建立共用 stems 快取：{SHARED_SHARED_STEMS_CACHE if 'SHARED_SHARED_STEMS_CACHE' in locals() else SHARED_STEMS_CACHE}")
    ...
    p190_cache = r"D:\Users\666\Desktop\UVR5 音檔\自動節拍器\.claude\worktrees\pass171-multi-variant-harness\outputs\pass190_default_pipeline_reverify\_shared_stems_cache\stems"
    src = p191_cache if os.path.exists(p191_cache) else (p190_cache if os.path.exists(p190_cache) else os.path.join(SOURCE_PROJECT_DIR, "stems"))
...
def main():
    ...
    print(f"[Pass192回驗] stems 重用方式：{stems_mode}")

    print(f"\n=== [Pass192回驗] 執行 target_stage=module3（含 Pass 192 過長小節拆分與防膨脹合併）===")
    t0 = time.time()
    ...
    print(
        f"Pass192 這次結果: {stats['total_measures']} 小節（差黃金基準 {diff['total_measures']:+}） / "
