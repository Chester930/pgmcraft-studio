r"""
Pass 197 — 全域 4/4 相位仲裁（階段 A 近距離衝突仲裁 + 階段 B 空隙插值）真實資料回驗
"""
...
REGRESSION_ROOT = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "outputs", "pass197_default_pipeline_reverify"
)
...
def ensure_shared_stems_cache() -> None:
    if os.path.exists(SHARED_STEMS_CACHE):
        return
    print(f"[Pass197回驗] 建立共用 stems 快取：{SHARED_STEMS_CACHE}")
    os.makedirs(os.path.dirname(SHARED_STEMS_CACHE), exist_ok=True)
    p196_cache = r"D:\Users\666\Desktop\UVR5 音檔\自動節拍器\.claude\worktrees\pass171-multi-variant-harness\outputs\pass196_default_pipeline_reverify\_shared_stems_cache\stems"
    src = p196_cache if os.path.exists(p196_cache) else os.path.join(SOURCE_PROJECT_DIR, "stems")
...
def main():
    ...
    print(f"[Pass197回驗] stems 重用方式：{stems_mode}")

    print(f"\n=== [Pass197回驗] 執行 target_stage=module3（全域相位仲裁 階段A+B）===")
    t0 = time.time()
    ...
    # 核對 Pass 194-196 曾出現不規則的位置這次的詳細樣貌，特別是
    # 任務書第 1 節分析過的 77.803s/78.542s 衝突組。
    known_problem_times = [8.041, 21.458, 32.382, 77.803, 78.542, 81.446, 93.802, 97.197, 108.652, 152.023]
    print("\nPass194-196 曾出現的問題點這次核對：")
    for t in known_problem_times:
        print(f"  --- {t}s 附近 ---")
        for m in measure_map:
            if abs(m["start_time"] - t) <= 3.0:
                beats_str = ", ".join(f"({b['beat']},{b['time']:.3f})" for b in m["beats"])
                irr = "irregular" if m.get("is_variable_length") else "normal"
                print(f"    measure={m['measure']} beat_count={m['beat_count']} [{irr}] start={m['start_time']:.3f} beats=[{beats_str}]")

    measure_map_json_path = os.path.join(project_dir, "reports", "measure_map.json")
    phase_reconciliation = []
    if os.path.exists(measure_map_json_path):
        with open(measure_map_json_path, "r", encoding="utf-8") as f:
            phase_reconciliation = json.load(f).get("phase_reconciliation", [])
    print(f"\nPass 197 仲裁/插值決策數：{len(phase_reconciliation)}")
    for decision in phase_reconciliation:
        print(f"  {decision}")

    print("\n" + "=" * 100)
    print(f"耗時: {elapsed:.1f}s")
    print(
        f"Pass197 這次結果: {stats['total_measures']} 小節（差黃金基準 {diff['total_measures']:+}） / "
        ...
        json.dump(
            {
                ...
                "phase_reconciliation": phase_reconciliation,
