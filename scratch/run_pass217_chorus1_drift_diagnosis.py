"""PASS-217: instrument the 95-130s Chorus 1 window where index-aligned
comparison against the golden measure_map shows a smooth, continuous,
positive drift accumulating from ~0.2s to over 1.3s -- committed bar
intervals in this stretch include a run of 16 consecutive bars at exactly
1.5430s, matching a previously-diagnosed-but-never-fixed self-referential
lock-in bug in DrumEvidenceBarSearchNode._expected_interval(committed)
(uses committed[-1]-committed[-2] as its own prediction target, so once a
slightly-too-long interval wins once, candidates matching THAT interval
keep winning, freezing the interval regardless of the true local tempo).

Captures full candidate lists + arbitration for every tick overlapping
this window, specifically to confirm whether committed candidates in the
locked stretch carry "expected_bar_interval" in evidence_sources (the
self-reference signature) and how their interval compares to golden's
true local bar length here (~1.45s, confirmed via
d:/.../reports/measure_map.json).
...
OUTPUT_ROOT = os.path.join(ROOT, "outputs", "pass217_chorus1_drift_diagnosis")
STEMS_SOURCE = os.path.join(ROOT, "outputs", "pass203_evidence_fusion_diagnosis", AUDIO_NAME, "stems")
TRACE_PATH = os.path.join(ROOT, "scratch", "pass217_chorus1_drift_trace.jsonl")
...
def _install_probes(trace):
    ...
    def wrapped_drum(self, blackboard):
        ...
        nonlocal last_drum_report, last_expected_interval
        last_drum_report = dict(blackboard.get_val("drum_bar_evidence_report", {}) or {})
        try:
            expected = self._expected_interval(committed_before) if hasattr(self, "_expected_interval") else None
        except Exception:
            expected = None
        last_expected_interval = {"expected_interval_used": expected, "committed_tail": committed_before[-3:]}
        return status
    ...
    def wrapped_commit(self, blackboard):
        ...
        if watch:
            ...
            trace.append(_json_safe({
                "window": window,
                "committed_before_last3": committed_before[-3:],
                "drum_evidence_report": last_drum_report,
                "drum_expected_interval_probe": last_expected_interval,
...
def main():
    if not os.path.exists(AUDIO_PATH):
        print(f"[PASS-217-chorus1drift] blocked: missing {AUDIO_PATH}")
        return 1
    reuse_stems_cache()
    ...
    print(f"[PASS-217-chorus1drift] elapsed={elapsed:.1f}s watched_ticks={len(trace)} trace={TRACE_PATH}")
    return 0
