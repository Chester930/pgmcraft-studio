r"""
Pass 197 Click 準確度量化分析——用真實鼓組音軌的 onset 偵測，核對每個
click 打點位置是否真的落在鼓組的真實擊點上（不是主觀聽感，是可驗證的
訊號分析）。改自 analyze_pass196_click_accuracy.py，指向 Pass197 輸出。
"""
...
PROJECT_DIR = (
    ...
    r"\outputs\pass197_default_pipeline_reverify"
...
TARGET_WINDOWS = [
    ...
    ("Pass197仲裁 77.803s vs 78.542s", 75.0, 82.0),
...
def main():
    ...
    for label, lo, hi in TARGET_WINDOWS:
        ...
        for t in window_clicks:
            ...
            hit = best_dist is not None and best_dist <= TOLERANCE_SEC
            total_checked += 1
            total_hit += int(hit)
            verdict = "✅ 有對應真實擊點" if hit else "❌ 附近沒有明顯鼓組擊點"
            print(
    ...
    print(f"\n{'='*90}\n總計：{total_hit}/{total_checked} 個目標區段內的 click 落在 50ms 容差內的真實鼓組擊點上\n{'='*90}")
