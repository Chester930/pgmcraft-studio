"""PASS-216: instrument the 66-86s Verse 1 window, which contains the last
4 of Verse 1's 13 interpolated bars (70.911526s, 75.289576s, 81.164224s,
84.020279s) -- the only stretch of World is Mine's 19 non-evidence bars
that has never been individually diagnosed (Pass 212's tick-by-tick trace
only covered 27-56s, the separate 34.5-54.9s alternating-skip cluster).

Captures full candidate lists + arbitration for every tick overlapping this
window so we can find why each "skipped" bar never clears commit on its
own, following the exact same methodology as
scratch/run_pass212_verse1_skip_diagnosis.py.
...
OUTPUT_ROOT = os.path.join(ROOT, "outputs", "pass216_verse1_tail_diagnosis")
STEMS_SOURCE = os.path.join(ROOT, "outputs", "pass203_evidence_fusion_diagnosis", AUDIO_NAME, "stems")
TRACE_PATH = os.path.join(ROOT, "scratch", "pass216_verse1_tail_trace.jsonl")
...
def main():
    if not os.path.exists(AUDIO_PATH):
        print(f"[PASS-216-v1tail] blocked: missing {AUDIO_PATH}")
        return 1
    reuse_stems_cache()
    ...
    print(f"[PASS-216-v1tail] elapsed={elapsed:.1f}s watched_ticks={len(trace)} trace={TRACE_PATH}")
    return 0
