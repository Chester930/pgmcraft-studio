r"""
Pass 193 — 相位連貫 4/4 拍重排與消除碎拍真實資料回驗
"""
...
REGRESSION_ROOT = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "outputs", "pass193_default_pipeline_reverify"
)
...
def ensure_shared_stems_cache() -> None:
    if os.path.exists(SHARED_STEMS_CACHE):
        return
    print(f"[Pass193回驗] 建立共用 stems 快取：{SHARED_STEMS_CACHE}")
    os.makedirs(os.path.dirname(SHARED_STEMS_CACHE), exist_ok=True)
    p192_cache = r"D:\Users\666\Desktop\UVR5 音檔\自動節拍器\.claude\worktrees\pass171-multi-variant-harness\outputs\pass192_default_pipeline_reverify\_shared_stems_cache\stems"
    p191_cache = r"D:\Users\666\Desktop\UVR5 音檔\自動節拍器\.claude\worktrees\pass171-multi-variant-harness\outputs\pass191_default_pipeline_reverify\_shared_stems_cache\stems"
    src = p192_cache if os.path.exists(p192_cache) else (p191_cache if os.path.exists(p191_cache) else os.path.join(SOURCE_PROJECT_DIR, "stems"))
...
def main():
    ...
    print(f"[Pass193回驗] stems 重用方式：{stems_mode}")

    print(f"\n=== [Pass193回驗] 執行 target_stage=module3（含 Pass 193 全曲 4/4 拍連貫重排與消除碎拍）===")
    t0 = time.time()
    ...
    print(
        f"Pass193 這次結果: {stats['total_measures']} 小節（差黃金基準 {diff['total_measures']:+}） / "
    ...
    # 複製最新的音檔至 Artifacts 供使用者隨時點擊試聽
    art_mix = r"C:\Users\666\.gemini\antigravity-ide\brain\34f7f467-3922-44d9-864f-a1d350fe8654\mix_with_click_pass193.wav"
    art_back = r"C:\Users\666\.gemini\antigravity-ide\brain\34f7f467-3922-44d9-864f-a1d350fe8654\backing_with_click_pass193.wav"
    if os.path.exists(click_path):
        shutil.copyfile(click_path, art_mix)
    backing_path = os.path.join(project_dir, "click", "backing_with_click.wav")
    if os.path.exists(backing_path):
        shutil.copyfile(backing_path, art_back)
