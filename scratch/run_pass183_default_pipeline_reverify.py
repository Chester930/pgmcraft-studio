r"""
Pass 180-183 — 累積修復後的預設管線真實資料回歸測試

目的：
Pass 180（ViterbiTempoSmoothingNode 治本修正）、Pass 181/182
（SteadyPercussionCountAnchorNode 新增+整軌比對）、Pass 183
（KickSnarePulseNode 補整軌交叉確認）都已經個別用合成資料驗證過，但從沒有
在真實歌曲、且是「完全預設設定」（GapReinforcementNode 維持 enabled=False，
不用任何 monkeypatch）的情況下，把四個修復疊加在一起跑過一次完整管線。

這支腳本對《World is Mine》重用 Pass 175 專案已經分離好的 stems/，跑一次
target_stage="module3" 的完整預設管線，讓使用者實際試聽比對，確認：

1. 18-20 秒附近（Pass 181/182 驗證過的 hi-hat 連續穩定擊點案例）第一拍
   位置是否真的被修正。
2. 整體跟黃金基準/先前幾次回歸的比較數字，有沒有其他段落退步。

用法：
    python scratch/run_pass183_default_pipeline_reverify.py
"""
...
REGRESSION_ROOT = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "outputs", "pass183_default_pipeline_reverify"
)
...
def ensure_shared_stems_cache() -> None:
    ...
    if os.path.exists(SHARED_STEMS_CACHE):
        return
    print(f"[Pass183回驗] 建立共用 stems 快取（僅此一次）：{SHARED_STEMS_CACHE}")
...
def main():
    ...
    print(f"[Pass183回驗] stems 重用方式：{stems_mode}")

    print(f"\n=== [Pass183回驗] 執行 target_stage=module3（完全預設設定，"
          f"含 Pass 180 Viterbi 修正 + Pass 181/182 SteadyPercussionCountAnchorNode "
          f"+ Pass 183 KickSnarePulseNode 整軌交叉確認；GapReinforcementNode 維持"
          f" enabled=False 預設）===")
    t0 = time.time()
    ...
    print(f"Pass180 重驗結果:  116 小節 / 169.69s / BPM跳動 0 / 不規則小節 1"
          f"（GapReinforcementNode enabled=True 測試用）")
    print(f"Pass183 這次結果: {stats['total_measures']} 小節（差黃金基準 {diff['total_measures']:+d}） / "
    ...
    print(f"\n完整報告：{report_path}")
    print(f"請實際試聽（特別注意 18-20 秒附近的第一拍位置）：{click_path}")
