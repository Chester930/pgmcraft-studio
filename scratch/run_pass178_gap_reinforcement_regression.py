r"""
Pass 178/179 — GapReinforcementNode 真實資料回歸測試 harness

目的：
GapReinforcementNode 目前只跑過合成音訊單元測試，從沒有在真實歌曲上跑過一次。
這支腳本對《World is Mine》這首歌，重用 Pass 175 專案已經分離好的 stems/
（複製到共用快取、junction 回去，避免重跑 Demucs），跑一次
target_stage="module3" 的完整正式管線，確認：

1. GapReinforcementNode 在真實資料上不會拋例外、能正常執行完。
2. 產出 reports/gap_reinforcement/blocks.json + beats.json 診斷輸出。
3. 跟 golden_benchmark 既有黃金基準比較整體品質分數的差異。

用法：
    python scratch/run_pass178_gap_reinforcement_regression.py
"""
...
REGRESSION_ROOT = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "outputs", "pass178_gap_reinforcement_regression"
)
...
def ensure_shared_stems_cache() -> None:
    """絕不直接 junction 回 Pass 175 的專案本身——那個資料夾這幾週一直在用（lanes/、
    marks.json 等衍生檔案還在持續更新），先真的複製一份到獨立快取，回歸測試才 link
    回這份快取，不會有寫壞正在使用中的專案的風險。"""
    if os.path.exists(SHARED_STEMS_CACHE):
        return
    print(f"[Pass178回歸] 建立共用 stems 快取（僅此一次）：{SHARED_STEMS_CACHE}")
...
def main():
    ...
    print(f"[Pass178回歸] stems 重用方式：{stems_mode}")

    print(f"\n=== [Pass178回歸] 執行 target_stage=module3（含 GapReinforcementNode）===")
    t0 = time.time()
    ...
    print(f"黃金基準: {GOLDEN_WORLD_IS_MINE_STATS['total_measures']} 小節 / "
          ...
          f"BPM {GOLDEN_WORLD_IS_MINE_STATS['avg_bpm']:.1f}")
    print(f"這次結果: {stats['total_measures']} 小節（差 {diff['total_measures']:+d}） / "
    ...
    report_path = os.path.join(REGRESSION_ROOT, "regression_report.json")
