...
REGRESSION_ROOT = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "outputs", "pass187_default_pipeline_reverify"
)
...
def ensure_shared_stems_cache() -> None:
    if os.path.exists(SHARED_STEMS_CACHE):
        return
    print(f"[Pass187回驗] 建立共用 stems 快取：{SHARED_STEMS_CACHE}")
...
def main():
    ...
    print(f"[Pass187回驗] stems 重用方式：{stems_mode}")

    print(f"\n=== [Pass187回驗] 執行 target_stage=module3（含 Pass 185 相位保護 + Pass 186 整軌確認放寬）===")
    t0 = time.time()
    ...
    print(f"Pass187 這次結果: {stats['total_measures']} 小節（差黃金基準 {diff['total_measures']:+d}） / "
