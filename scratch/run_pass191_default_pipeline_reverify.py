r"""
Pass 191 — _relabel_beat_numbers 相位連貫修復後的預設管線真實資料回驗
"""
...
REGRESSION_ROOT = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "outputs", "pass191_default_pipeline_reverify"
)
...
def ensure_shared_stems_cache() -> None:
    if os.path.exists(SHARED_STEMS_CACHE):
        return
    print(f"[Pass191回驗] 建立共用 stems 快取：{SHARED_STEMS_CACHE}")
    ...
    src = p190_cache if os.path.exists(p190_cache) else os.path.join(SOURCE_PROJECT_DIR, "stems")
...
def main():
    ...
    print(f"[Pass191回驗] stems 重用方式：{stems_mode}")

    print(f"\n=== [Pass191回驗] 執行 target_stage=module3（含 Pass 191 相位連貫重標號）===")
    t0 = time.time()
    ...
    print(f"Pass191 這次結果: {stats['total_measures']} 小節（差黃金基準 {diff['total_measures']:+d}） / "
