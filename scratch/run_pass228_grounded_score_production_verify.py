"""PASS-228: real-pipeline verification of the grounded headline quality
score (_score_beat_grid_grounded) -- confirms real kick_anchors/kick-stem
wiring works end-to-end and reports the new score baseline, which is NOT
comparable to any score cited in Pass 197-227 (see
docs/PASS-228-BEAT-GRID-QUALITY-SCORE-REAL-GROUNDING-TASK.md).

Identical harness to scratch/run_pass211_promoted_production_verify.py
(no monkeypatching, real stall_limit default, reuses cached stems,
barstart_v2_promotion_approved=True so the promotion decision path also
runs end-to-end).
"""
...
OUTPUT_ROOT = os.path.join(ROOT, "outputs", "pass228_grounded_score_production_verify")
...
def main():
    if not os.path.exists(AUDIO_PATH):
        print(f"[PASS-228] blocked: missing {AUDIO_PATH}")
        return 1
    reuse_stems_cache()
    os.makedirs(OUTPUT_ROOT, exist_ok=True)

    print("[PASS-228] running target_stage=module3 with grounded quality score wiring")
    started = time.time()
    ...
    output = {
        "pass": "Pass 228 (grounded quality score production verify)",
        "elapsed_sec": elapsed,
        ...
        "barstart_v2_quality_score": v2_report.get("quality_score"),
    ...
    print(f"[PASS-228] report: {report_path}")
    return 0
