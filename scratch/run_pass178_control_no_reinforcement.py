r"""
Pass 178 — 對照組：GapReinforcementNode 暫時停用的版本

目的：
單獨跑一次 GapReinforcementNode 啟用時的結果，沒辦法判斷「跟黃金基準的差異」
是這個新節點造成的，還是這次重新分軌/既有機制本來就有的變異。這支腳本在
GapReinforcementNode 被暫時註解掉（見 pgm_craft/workflow/beat_tracking_bt.py
的 TEMP DISABLED 註記，跑完這次對照組後會用 git checkout 還原）的狀態下，
用同一份來源音檔重跑一次，輸出到獨立的資料夾，供跟
outputs/pass178_gap_reinforcement_regression/ 的結果直接比對。

用法：
    python scratch/run_pass178_control_no_reinforcement.py
"""
...
REGRESSION_ROOT = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "outputs", "pass178_control_no_reinforcement"
)
...
def ensure_shared_stems_cache() -> None:
    if os.path.exists(SHARED_STEMS_CACHE):
        return
    print(f"[Pass178對照組] 建立共用 stems 快取（僅此一次）：{SHARED_STEMS_CACHE}")
...
def main():
    ...
    print(f"[Pass178對照組] stems 重用方式：{stems_mode}")

    print(f"\n=== [Pass178對照組] 執行 target_stage=module3（GapReinforcementNode 暫時停用）===")
    t0 = time.time()
    ...
    print(f"這次結果（無 GapReinforcementNode）: {stats['total_measures']} 小節（差 {diff['total_measures']:+d}） / "
    ...
    report_path = os.path.join(REGRESSION_ROOT, "control_report.json")
