r"""
Pass 196 — _prune_ghost_downbeats / _merge_short_measures 尊重保護區段 真實資料回驗
"""
...
REGRESSION_ROOT = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "outputs", "pass196_default_pipeline_reverify"
)
...
def ensure_shared_stems_cache() -> None:
    if os.path.exists(SHARED_STEMS_CACHE):
        return
    print(f"[Pass196回驗] 建立共用 stems 快取：{SHARED_STEMS_CACHE}")
    os.makedirs(os.path.dirname(SHARED_STEMS_CACHE), exist_ok=True)
    p195_cache = r"D:\Users\666\Desktop\UVR5 音檔\自動節拍器\.claude\worktrees\pass171-multi-variant-harness\outputs\pass195_default_pipeline_reverify\_shared_stems_cache\stems"
    src = p195_cache if os.path.exists(p195_cache) else os.path.join(SOURCE_PROJECT_DIR, "stems")
...
def main():
    ...
    print(f"[Pass196回驗] stems 重用方式：{stems_mode}")

    print(f"\n=== [Pass196回驗] 執行 target_stage=module3（ghost-pruning / merge 尊重保護區段）===")
    t0 = time.time()
    ...
    # 核對 Pass 194/195 曾出現不規則的 9 個位置這次的詳細樣貌
    # （不只看是否還在 irregular 清單，還要看 beat_count/內容有沒有變）。
    known_problem_times = [8.041, 21.458, 32.382, 77.803, 81.446, 93.802, 97.197, 108.652, 152.023]
    print("\nPass194/195 曾出現的 9 個問題點這次核對：")
    ...
    print(
        f"Pass196 這次結果: {stats['total_measures']} 小節（差黃金基準 {diff['total_measures']:+}） / "
