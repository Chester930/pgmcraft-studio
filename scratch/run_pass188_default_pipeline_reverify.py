...
REGRESSION_ROOT = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "outputs", "pass188_default_pipeline_reverify"
)
...
def ensure_shared_stems_cache() -> None:
    if os.path.exists(SHARED_STEMS_CACHE):
        return
    print(f"[Pass188回驗] 建立共用 stems 快取：{SHARED_STEMS_CACHE}")
    os.makedirs(os.path.dirname(SHARED_STEMS_CACHE), exist_ok=True)
    p187_cache = r"D:\Users\666\Desktop\UVR5 音檔\自動節拍器\.claude\worktrees\pass171-multi-variant-harness\outputs\pass187_default_pipeline_reverify\_shared_stems_cache\stems"
    src = p187_cache if os.path.exists(p187_cache) else os.path.join(SOURCE_PROJECT_DIR, "stems")
...
def main():
    ...
    print(f"[Pass188回驗] stems 重用方式：{stems_mode}")

    print(f"\n=== [Pass188回驗] 執行 target_stage=module3（含 Pass 188 短小節合併）===")
    t0 = time.time()
    ...
    print(f"Pass188 這次結果: {stats['total_measures']} 小節（差黃金基準 {diff['total_measures']:+d}） / "
