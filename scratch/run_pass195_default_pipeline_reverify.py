r"""
Pass 195 — MeasureMapNode 相位補全改為只局部修復真的不規則區段 真實資料回驗
"""
...
REGRESSION_ROOT = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "outputs", "pass195_default_pipeline_reverify"
)
...
def ensure_shared_stems_cache() -> None:
    if os.path.exists(SHARED_STEMS_CACHE):
        return
    print(f"[Pass195回驗] 建立共用 stems 快取：{SHARED_STEMS_CACHE}")
    os.makedirs(os.path.dirname(SHARED_STEMS_CACHE), exist_ok=True)
    p194_cache = r"D:\Users\666\Desktop\UVR5 音檔\自動節拍器\.claude\worktrees\pass171-multi-variant-harness\outputs\pass194_default_pipeline_reverify\_shared_stems_cache\stems"
    src = p194_cache if os.path.exists(p194_cache) else os.path.join(SOURCE_PROJECT_DIR, "stems")
...
def main():
    ...
    print(f"[Pass195回驗] stems 重用方式：{stems_mode}")

    print(f"\n=== [Pass195回驗] 執行 target_stage=module3（MeasureMapNode 只局部修復真的不規則區段）===")
    t0 = time.time()
    ...
    # 核對 Pass 194 曾出現不規則的 9 個位置（跟保護區段無關的那批），
    # 這次是否已經不再出現於不規則清單中。
    known_problem_times = [8.041, 21.458, 32.382, 77.803, 81.446, 93.802, 97.197, 108.652, 152.023]
    irregular_times = [m["start_time"] for m in irregulars]
    print("\nPass194 曾出現的 9 個問題點這次核對：")
    for t in known_problem_times:
        still_irregular = any(abs(t - it) < 1.0 for it in irregular_times)
        print(f"  {t}s -> {'❌ 依然不規則' if still_irregular else '✅ 已修復'}")

    print("\n" + "=" * 100)
    print(f"耗時: {elapsed:.1f}s")
    print(
        f"Pass195 這次結果: {stats['total_measures']} 小節（差黃金基準 {diff['total_measures']:+}） / "
